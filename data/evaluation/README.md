...
| `heldout/v2-multidish/` | v2-multidish | Do not tune against its results | 40 episodes drafted in a sealed packet and revised on the owner's notes in two review rounds ([provenance](heldout/v2-multidish/provenance.json), [review](heldout/v2-multidish/review.json)); frozen, with pools drawn by script and every label proven from catalog facts |
...
| `dev/v3-meal-day-week/episodes/` | v3-meal-day-week | May be inspected and used while developing; not held-out evidence | Sixteen developer episodes of several meals a day, per-meal compositions, per-day nutrition and shape changes by request ([protocol](../../docs/evaluation/protocol-v3-meal-day-week.md)); pools drawn by `python -m app.evaluation.multidish_pool`, labels checked by `python -m app.evaluation.multidish_labels`, run by `python -m app.evaluation.meal_day_week_runner`. No held-out set yet |
...
The `planning-catalog-v2.1` files are generated by `scripts/derive_catalog_labels.py`
(CI checks them with `--check`). It relabels the v1 scenarios for the larger
catalog from the catalog files alone, running no system under test, and checks
the new scenarios' labels the same way; the record of what changed and why is
`docs/evaluation/catalog-v2.1-labels.json`. The v1 files are not changed.
