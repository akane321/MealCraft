...
def test_subcent_budget_proposals_are_not_rounded_into_compliance(mixed):
    problem = mixed_packet().model_copy(update={"purchase_budget_sgd": 3})
    with pytest.raises(ValueError, match="whole cents"):
        propose_relaxations(problem, (RelaxationChange("subcent", "purchase_budget", 3.499, 1),), mixed_packages=mixed)
    with pytest.raises(ValueError, match="whole cents"):
        propose_relaxations(problem.model_copy(update={"purchase_budget_sgd": 3.499}), (), mixed_packages=mixed)
