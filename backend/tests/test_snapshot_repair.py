...
def test_degraded_snapshot_keeps_detached_evidence_without_using_prices():
    full = packet()
    problem = full.model_copy(update={"products": []})
    saved = FixtureProvider(full.products).retrieve(())
    saved.trace.status = "degraded"

    class DegradedProvider:
        def retrieve(self, demands):
            return saved

    result = solve_with_repair(problem, DegradedProvider())
    assert result.stop_reason == "provider_degraded"
    assert result.solution.status == "needs_data"
    assert result.attempts[-1].products == tuple(full.products)
    saved.products[0].price_sgd = 999
    saved.trace.query = "changed after return"
    assert result.attempts[-1].products[0].price_sgd != 999
    assert result.attempts[-1].retrieval.query != saved.trace.query


def test_invalid_round_limits_are_rejected_before_provider_call():
    import pytest

    for limit in (True, False, -1, 1.5):
        provider = FixtureProvider([])
        with pytest.raises(ValueError, match="nonnegative integer"):
            solve_with_repair(packet(), provider, max_rounds=limit)
        assert provider.calls == []
