...
def main():
    ...
    if args.operation == "packages":
        ...
        if args.operation == "plan":
            solution = BeamPlanner(BeamLimits(args.width, args.limit)).solve(problem)
            output = solution.model_dump(mode="json")
        elif args.operation == "mixed-plan":
            output = asdict(solve_mixed_beam(problem, limits=BeamLimits(args.width, args.limit)))
        elif args.operation == "oracle":
            output = asdict(exhaustive_assignments(problem, max_combinations=args.limit))
        elif args.operation == "mixed-oracle":
            output = asdict(exhaustive_mixed_plan(problem, max_assignments=args.limit, use_cp_sat=args.cp_sat))
        else:
            ...
            if args.operation == "relax":
                ...
                result = (
                    ...
                    if args.mixed_packages
                        ...
                        FileSnapshots(options),
                        max_rounds=args.rounds,
                        planner=BeamPlanner(BeamLimits(args.width, args.limit)),
                    )
                )
                output = {
