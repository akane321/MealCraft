...
Verification from backend: `python -m pytest tests/test_constraint_compiler.py tests/test_planning_v2.py` (21 passed). Ruff check and format were run on the two Python files. Tests cover each filter, deterministic matrix ordering, input preservation, nutrition scope, soft preferences, inclusive numeric boundaries, tolerance, and safety conflicts in optional locked slots. These are component tests, not an end-to-end evaluation.
