...
2. Record the owner's sampled review of the release v2 FairPrice mapping
   (40 ingredients, fixed seed), then re-run the catalog-dependent measurements
   in this document against the larger catalog.
3. Keep release recipes whose ingredient list is incomplete out of plans: some
   RecipeNLG sources list only part of a dish (a crab quiche listing only its
   crust), so their derived allergens and dietary tags miss what the title
   names. A title keyword scan flags about 66 of 9,048, some of them false
   positives.
