...
- 30 curated recipes and 34 normalized ingredients in the recorded catalog, plus 9,046 release v2 recipes imported beside them (slugs prefixed `v2-`).
...
2. Re-run the catalog-dependent measurements in this document against the
   larger catalog.
3. Carry the import-time corrections into the next data release: the six
   stricter ingredient allergens and the incomplete-list and tag checks in
   `backend/app/data/release_v2.py` belong in the release build, where the
   release gate can count them.
4. Complete nutrition-target and elastic-preference semantics and evidence.
5. Test FairPrice live/cache/fixture degradation against real changes.
6. Integrate the verified Planning v2 components into the product path, then
   evaluate Beam parameters, retrieve-repair and package optimization without
   overstating global optimality or infeasibility.
7. Extend the merged synchronous `AgentRun` into asynchronous pause/resume/retry
...
8. Expand dynamic-replanning stress cases and measure unnecessary disruption.
9. Increase browser coverage and prepare a repeatable demonstration path.
10. Add member-management routes, expand the cross-household isolation matrix as
   private resources grow, and complete household collaboration plus account
   lifecycle flows.
11. Raise the discriminating power of the held-out evaluation before the final
...
12. Progressively close the final-design gaps documented above rather than
