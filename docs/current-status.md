...
| Replanning | Preview, confirm/discard, plan revision, event history, local meal changes, price and Shopping List deltas. Shape changes by request (ADR-0046 section 2): add or drop a meal for the rest of the week, change a meal's dishes, or add or take away a dish in one meal. Only the affected meals are planned again, with the budget the rest of the week leaves; taking a dish away keeps the others at their larger share; cooked and locked meals never change. A week-wide change applies to this week, and the assistant then asks whether new weeks should plan the same way, saving it to the profile only on a yes | Shape changes are read by rules (English and Chinese), not by the model. Adding a dish to one meal plans that whole meal again rather than only the new dish. Broader preference and stress-event semantics remain partial |
